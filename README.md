...
# C# SDK for the FINBOURNE Notifications API
...
This is the C# SDK for the FINBOURNE Notifications API, part of the [LUSID by FINBOURNE](https://www.finbourne.com/lusid-technology) platform. To use it you'll need a LUSID account - [sign up for free at lusid.com](https://www.lusid.com/app/signup).

The notification application provides notifications when LUSID events occur. Various types of notification can be configured, including email, SMS, webhook, Amazon SQS and Azure Service Bus notifications - see https://support.lusid.com/knowledgebase/article/KA-01817/ to learn more.
...
The NuGet package for the FINBOURNE Notifications API SDK can installed from https://www.nuget.org/packages/Finbourne.Notifications.Sdk using the following
...
You can send your requests to the FINBOURNE Notifications API via a proxy, by setting `FBN_PROXY_ADDRESS`. If your proxy has basic auth enabled, you must also set `FBN_PROXY_USERNAME` and `FBN_PROXY_PASSWORD`.
...
You can send your requests to the FINBOURNE Notifications API via a proxy, by adding a proxy section. If your proxy has basic auth enabled, you must also supply a `username` and `password` in this section.
